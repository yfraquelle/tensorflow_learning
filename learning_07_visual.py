...
ax.scatter(x_data,y_data) # 以点的形式显示在图片上
plt.ion() # 需要指定不终止程序
plt.show() # 本会将整个程序暂停，画出
...
#         lines=[]
        try:
            ...
            pass
        prediction_vlaue=sess.run(prediction,feed_dict={xs:x_data})
        lines=ax.plot(x_data,prediction_vlaue,'r-',lw=5) # 以曲线的形式画出，红色-线，宽度为5
        plt.pause(0.1) # 每次暂停0.1秒