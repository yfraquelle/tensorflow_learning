...
def add_layer(inputs,in_size,out_size,name_layer,activation_function=None):# None means linear
    ...
    with tf.name_scope(layer_name): # 大的框架
        with tf.name_scope('weights'): # 小的部件
            Weights=tf.Variable(tf.random_normal([in_size,out_size]),name='W')# in_size行，out_size列的矩阵
            tf.summary.histogram(' ',Weights) # 想要用histogram查看的变量下加这句
        with tf.name_scope('biases'): # 小的部件
            biases=tf.Variable(tf.zeros([1,out_size])+0.1,name='b')# 向量，初始值最好不为0
...
# 每加一个图层都会加一个框架
# 隐藏层
l1=add_layer(xs,1,10,name_layer=1,activation_function=tf.nn.relu) # 输入数据为x_data、输入一个数据、输出10个数据（隐藏层有10个神经元）
# 输出层
prediction=add_layer(l1, 10, 1, name_layer=2,activation_function=None) # 线性函数

# 预测之前，计算预测值与真实值的差别
with tf.name_scope('loss'):
    loss=tf.reduce_mean(tf.reduce_sum(tf.square(ys-prediction),reduction_indices=[1])) # 实际算出的是每个例子的平方，需要求和，再求出平均值
    tf.summary.scalar('loss',loss) # 用scalar查看loss，如果loss在减小说明神经网络学到了东西
...
for i in range(1000):
    ...
    if i%50==0:
        ...
        writer.add_summary(result,i) # 每隔50步记一个点
